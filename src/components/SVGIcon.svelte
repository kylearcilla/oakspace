<script lang="ts">
  ...
  $: {
    if ($themeState && !options.iconColor) {
      iconColor = `rgb(${getThemeStyling("textColor1")})`
    }
  }
...
{#if icon === Icon.Settings}
    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="18" id={options.id ?? ""}>
      <g fill={iconColor} stroke={iconColor} opacity={options.opacity ?? 1} stroke-linecap="round" transform="translate(0 8.5)">
...
{:else if icon === Icon.Pin}
      <svg xmlns="http://www.w3.org/2000/svg" width="9" height="11" viewBox="0 0 9 11" fill="none" style={`opacity: ${options.opacity ? options.opacity : 1};`} >
          <path 
