<script>
    ...
    const { header, sections } = GOALS_ARTICLES["systems-mindsets"]
    const { title, summary, image } = header
    const nextArticles = READ_NEXT["systems-mindsets"]
            ...
            <ul>
                {#each sections as section}
                    {#if section.type === 'h1' || section.type === 'h2'}     
                        {@const h2 = section.type === 'h2'}
                        <li 
                            class={activeSection === section.href?.slice(1) ? 'active' : ''}
                            style:margin-left={h2 ? '0.5rem' : '0'}
                        >
                            <a href={`#${section.href?.slice(1)}`}>
                                {section.content}
                            </a>
                        </li>
                    {/if}
